...
from workshop.serializers import BookingSerializer, CancelBookingSerializer, WorkshopBookingCreateSerializer, WorkshopSerializer
...
from .models import WorkshopSharing
...
class BookingListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        participant_id = self.kwargs.get(self.lookup_url_kwarg)
        queryset = Booking.objects.filter(Participant_id=participant_id)
        return queryset

class WorkshopBookingCreateAPIView(generics.CreateAPIView):
    permission_classes = [permissions.AllowAny]
    serializer_class = WorkshopBookingCreateSerializer

    def get_queryset(self):
        return Booking.objects.none()

class CancelBookingView(generics.CreateAPIView):
...
def workshop_sharing(request, workshop_id):
    try:
        sharing = WorkshopSharing.objects.filter(Workshop_id_id=workshop_id)
        data = []
        for item in sharing:
            data.append({
                'id': item.id,
                ...
                'workshop_id': item.Workshop_id_id
            })
        return JsonResponse(data, safe=False)
    except WorkshopSharing.DoesNotExist:
        return JsonResponse({'error': 'Workshop sharing not found'}, status=404)
